...
const arrYear = ref([])
//Out
const wasteCityOut_year = ref([])
const wasteCityOut_mon = ref([])
//Handle
const wasteCityHandle_year = ref([])
const wasteCityHandle_mon = ref([])
//Out
const wasteAllOut_year = ref([])
const wasteAllOut_mon = ref([])
//Handle
const wasteAllHandle_year = ref([])
const wasteAllHandle_mon = ref([])
onMounted(() => {
    //執行機關一般廢棄物產生量,資料年間 110~112
    ...
    .then(dataCity_out =>{
        ...
        DataCity_out.forEach((item) => {
            let CityOut_year = { 統計期: '', 統計區: '', 總產生量: '', 一般垃圾量: '', 資源垃圾量: '', 廚餘量: '' }
            let CityOut_mon = { 統計期_年: '', 統計期_月: '', 統計區: '', 總產生量: '', 一般垃圾量: '', 資源垃圾量: '', 廚餘量: '' }
            let day = item.item1.split(" ")
            if ((Number(day[0].split("年")[0]) >= 110) && (Number(day[0].split("年")[0]) <= 112)) {
                if (Object.keys(day).length == 2){
                    CityOut_mon.統計期_年 = day[0].split("年")[0];
                    CityOut_mon.統計期_月 = day[1].split("月")[0];
                    CityOut_mon.統計區 = item.item2;
                    CityOut_mon.總產生量 = item.value1;
                    CityOut_mon.一般垃圾量 = item.value2;
                    CityOut_mon.資源垃圾量 = item.value3;
                    CityOut_mon.廚餘量 = item.value4;
                    wasteCityOut_mon.value.push(CityOut_mon);
                }
                if (Object.keys(day).length == 1){
                    CityOut_year.統計期 = day[0].split("年")[0];
                    CityOut_year.統計區 = item.item2;
                    CityOut_year.總產生量 = item.value1;
                    CityOut_year.一般垃圾量 = item.value2;
                    CityOut_year.資源垃圾量 = item.value3;
                    CityOut_year.廚餘量 = item.value4;
                    wasteCityOut_year.value.push(CityOut_year);
                }
            }
            // console.log(wasteCityOut_year);
            // console.log(wasteCityOut_mon);
            wasteCityOut_year.value.forEach((item) => {
                if (!arrYear.value.includes(item.統計期)) {
                    arrYear.value.push(item.統計期);
                }
            });
        });
    })
    ...
    .then(dataCity_handle =>{
        ...
        DataCity_handle.forEach((item) => {
            let CityHandle_year = { 統計期: '', 統計區: '', 總處理量: '', 回收再利用量: '', 焚化量: '', 衛生掩埋量: '', 其他處理量: '' }
            let CityHandle_mon = { 統計期_年: '', 統計期_月: '', 統計區: '', 總處理量: '', 回收再利用量: '', 焚化量: '', 衛生掩埋量: '', 其他處理量: '' }
            let day = item.item1.split(" ")
            if ((Number(day[0].split("年")[0]) >= 110) && (Number(day[0].split("年")[0]) <= 112)) {
                if (Object.keys(day).length == 2){
                    CityHandle_mon.統計期_年 = day[0].split("年")[0];
                    CityHandle_mon.統計期_月 = day[1].split("月")[0];
                    CityHandle_mon.統計區 = item.item2;
                    CityHandle_mon.總處理量 = item.value1;
                    CityHandle_mon.回收再利用量 = item.value2;
                    CityHandle_mon.焚化量 = item.value3;
                    CityHandle_mon.衛生掩埋量 = item.value4;
                    CityHandle_mon.其他處理量 = item.value5;
                    wasteCityHandle_mon.value.push(CityHandle_mon);
                }
                if (Object.keys(day).length == 1){
                    CityHandle_year.統計期 = day[0].split("年")[0];
                    CityHandle_year.統計區 = item.item2;
                    CityHandle_year.總處理量 = item.value1;
                    CityHandle_year.回收再利用量 = item.value2;
                    CityHandle_year.焚化量 = item.value3;
                    CityHandle_year.衛生掩埋量 = item.value4;
                    CityHandle_year.其他處理量 = item.value5;
                    wasteCityHandle_year.value.push(CityHandle_year);
                }
            }
            // console.log(wasteCityHandle_year);
            // console.log(wasteCityHandle_mon);
        });
    })
    ...
    .then(dataAll_out =>{
        ...
        DataAll_out.forEach((item) => {
            let AllOut_year = { 統計期: '', 總產生量: '', 一般垃圾量: '', 資源垃圾量: '', 廚餘量: '' }
            let AllOut_mon = { 統計期_年: '', 統計期_月: '', 總產生量: '', 一般垃圾量: '', 資源垃圾量: '', 廚餘量: '' }
            let day = item.item1.split(" ")
            if (Object.keys(day).length == 2){
                AllOut_mon.統計期_年 = day[0].split("年")[0];
                AllOut_mon.統計期_月 = day[1].split("月")[0];
                AllOut_mon.總產生量 = item.value1;
                AllOut_mon.一般垃圾量 = item.value2;
                AllOut_mon.資源垃圾量 = item.value3;
                AllOut_mon.廚餘量 = item.value4;
                wasteAllOut_mon.value.push(AllOut_mon);
            }
            if (Object.keys(day).length == 1){
                AllOut_year.統計期 = day[0].split("年")[0];
                AllOut_year.總產生量 = item.value1;
                AllOut_year.一般垃圾量 = item.value2;
                AllOut_year.資源垃圾量 = item.value3;
                AllOut_year.廚餘量 = item.value4;
                wasteAllOut_year.value.push(AllOut_year);
            }
            // console.log(wasteAllOut_year);
            // console.log(wasteAllOut_mon);
        });
    })
    ...
    .then(dataAll_handle =>{
        ...
        DataAll_handle.forEach((item) => {
            let AllHandle_year = { 統計期: '', 總處理量: '', 回收再利用量: '', 焚化量: '', 衛生掩埋量: '', 其他處理量: '' }
            let AllHandle_mon = { 統計期_年: '', 統計期_月: '', 總處理量: '', 回收再利用量: '', 焚化量: '', 衛生掩埋量: '', 其他處理量: '' }
            let day = item.item1.split(" ")
            if (Object.keys(day).length == 2){
                AllHandle_mon.統計期_年 = day[0].split("年")[0];
                AllHandle_mon.統計期_月 = day[1].split("月")[0];
                AllHandle_mon.總處理量 = item.value1;
                AllHandle_mon.回收再利用量 = item.value2;
                AllHandle_mon.焚化量 = item.value3;
                AllHandle_mon.衛生掩埋量 = item.value4;
                AllHandle_mon.其他處理量 = item.value5;
                wasteAllHandle_mon.value.push(AllHandle_mon);
            }
            if (Object.keys(day).length == 1){
                AllHandle_year.統計期 = day[0].split("年")[0];
                AllHandle_year.總處理量 = item.value1;
                AllHandle_year.回收再利用量 = item.value2;
                AllHandle_year.焚化量 = item.value3;
                AllHandle_year.衛生掩埋量 = item.value4;
                AllHandle_year.其他處理量 = item.value5;
                wasteAllHandle_year.value.push(AllHandle_year);
            }
            // console.log(wasteAllHandle_year);
            // console.log(wasteAllHandle_mon);
        });
    })
})
console.log(arrYear);
...
function cityClick(city) {
    const clickedCity = city.currentTarget.getAttribute('data-name')
...
const option = ref({
    ...
    },
    {
        ...
        data: [
            { value: 100, name: '回收再利用量' },
            { value: 100, name: '焚化量' },
            { value: 100, name: '衛生掩埋量' },
            { value: 100, name: '其他處理量' }
        ]
    }]
})

function upDateCity(params) {
    let catchData = [selectedCity, selectedYear, selectedMonth]
    // ALL
    if (catchData[0] == ""){
        // year
        const output = null
        if (catchData[2] == ""){
            output = wasteAllOut_year.filter((data) => {
                return data.統計期 === selectedYear
            })
            console.log(output);
            option.value.series[0].data = [
                { value: output, name: '一般垃圾量' },
                { value: 100, name: '資源垃圾量' },
                { value: 100, name: '廚餘量' }
            ]
        }else{
            output = wasteAllOut_mon.filter((data) => {
                return data.統計期_年 === selectedYear && data.統計期_月 === selectedMonth
            })
            console.log(output);
        }
    }
...
<template>
    <P>各縣市歷年一般廢棄物資料{{ this.yearOption }}</P>
    <div class="total">
        <div class="svgTaiwan">
            <svg viewBox="0 0 595.28 841.89">
                <g id="PenghuCounty" data-name="澎湖縣" class="city" @click="cityClick">
                    <path d="M63.72,439.41l-.12,1.71,1.46-.61-1.34-1.1Z"/>
                    <path d="M66.28,443.68l-2.93,1.59-1.1-.49.24,1.58,3.66-.24,2.8-1.71-.24-.85L67,443.31l-.73.37Z"/>
