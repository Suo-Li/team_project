...
function cityClick(city) {
    ...
    let selItemOut = dataOut.value.find(item => item.統計期_年 === selectedYear.value && Number(item.統計期_月) === selectedMonth.value && item.統計區 === selectedCity.value);
    console.log(selItemOut);
    if(selItemOut){
        option.value.series[0].data = [
            { value: selItemOut.一般垃圾量, name: '一般垃圾量' },
            { value: selItemOut.資源垃圾量, name: '資源垃圾量' },
            { value: selItemOut.廚餘量, name: '廚餘量' },
        ]
    }
    let selItemHandle = dataHandle.value.find(item => item.統計期_年 === selectedYear.value && Number(item.統計期_月) === selectedMonth.value && item.統計區 === selectedCity.value);
    console.log(selItemHandle);
    if(selItemHandle){
        option.value.series[1].data = [
            { value: selItemHandle.回收再利用量, name: '回收再利用量' },
            { value: selItemHandle.焚化量, name: '焚化量' },
            { value: selItemHandle.衛生掩埋量, name: '衛生掩埋量' },
            { value: selItemHandle.其他處理量, name: '其他處理量' },
        ]
    }
}
function twClick() {
